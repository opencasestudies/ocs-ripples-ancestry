...
This case study explores human {{< glossary "genomic variation" add_to_table=true >}}, especially with respect to {{< glossary ancestry add_to_table=true >}}.
...
Most of a person's genome is the same as another person's. All humans need the same proteins to function. But very small parts of our genome, especially in areas that do not code for proteins, can vary across individuals. This genomic variation is caused by changes in the nucleotides ssequences that can cause differences in how a protein functions or the amount of protein created. Some variations can change the function of a protein so severly that it stops doing its intended function, while others can increase or decrease the amount of protein that is produced.
...
These variations in a genome are caused by mutations. Mutations are any changes in the DNA or nucleotides that make up the genome. These variations can be caused by a change of a single nucleotide ("point mutations") or chnages involving more than one nucleotide.
...
An example of a single nucleotide change would be changing a base from C → T or adding or removing a single nucleotide. If a point mutation occurs fairly often within a population such that it is observed with a common frequency, it is termed a single-nucleotide polymorphism or SNP.
