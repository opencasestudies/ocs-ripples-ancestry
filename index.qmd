...
#' Create glossary table from glossary.yml
#' @param path Path to glossary.yml
#' @return HTML table as an htmltools::HTML object
glossary_table <- function(path = "glossary.yml") {

  # read YAML as named list
  ...
  # build table rows
  rows <- Map(function(term, def) {
    tags$tr(
      tags$td(term),
      tags$td(htmltools::HTML(def))
    )
  }, names(g_sorted), g_sorted)

  # full table
  tbl <- tags$table(
    class = "glossary_table",
    tags$tr(
      tags$th("Term"),
      tags$th("Definition")
    ),
