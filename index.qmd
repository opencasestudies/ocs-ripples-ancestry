...
Let's take a look at a few now. We can use the `slice` function to get the data for a single row. Say we want the 500th row we can do so like this:

```{r}
slice(ancestry_props, 500) 
```

We can see that most of the genomic variation appears to match up wtih cluster 3 and 5. these have the largest numbers, with k5 showing a much higher proportion that is pretty close to 1. Therefore most of the genetic variation of this individual 
