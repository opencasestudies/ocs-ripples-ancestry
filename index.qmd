...
* By scrolling, we see that there are 1914 rows of data (including the header), meaning there is data for 1913 individuals. 
  * If you didn't want to scroll to see how much data there is, you could enter `=COUNTA(A1:A1914)` into a spreadsheet cell off to the side and it would return the same number. The [`COUNTA` function in Google sheets](https://support.google.com/docs/answer/3093991?hl=en) returns how many values are in a dataset. 

* To see the unique population names in the `pop` column, in another spreadsheet cell off to side, enter `=UNIQUE(A1:A1914)` and it will list the unique population names in that column. 

::: {.question_block}

How would you change the `=UNIQUE(A1:A1914)` function to show the number of unique populations in the dataset?
...
Use `=COUNTA(UNIQUE(A1:A1914))` and that will return the number of unique populations within that column + 1 because it is also counting the `pop` header as a unique value in the A column.

</details>

:::

The wrangling steps that are being performed in the R version of this case study have already been done for the dataset that was imported. However, we will discuss the options you have for performing those sorts of wrangling steps in Google Sheets. 
...
### Single Substitution Across a Range
...
### Multiple Substitutions Across a Range
...
If you needed to filter out rows ...

If you needed to order / sort values ...

If you needed to rearrange the data from a long format to a wide format ...
